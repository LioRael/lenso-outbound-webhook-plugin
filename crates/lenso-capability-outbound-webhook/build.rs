...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update Outbound Webhook snapshot: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("Outbound Webhook Descriptor or Schemas are stale: {error}")
        });
    }
    check_projection(
    ...
    .unwrap_or_else(|error| panic!("Outbound Webhook Rust projection is stale: {error}"));
}
